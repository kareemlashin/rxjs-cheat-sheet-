...
import { delay, from, map, of, reduce, filter, throwError, interval, timer, range, concat, merge, forkJoin, zip } from 'rxjs';
import { catchError, debounceTime, distinct, skip, take, toArray, count, max, min, retry, switchMap, mergeMap, flatMap, combineAll, concatAll, mergeAll, first, last, startWith, groupBy, pluck, concatMap } from 'rxjs/operators';
...
export class LearnService {
    ...
    opsMap$ = of(...this.users).pipe(map((data) => { return { ...data, age: data.age + 1 } }));
    ...
    opsCombineLatest$ = combineLatest(this.timerOne$, this.timerTwo$, this.timerThree$)
    // -------------------------------------- // 
